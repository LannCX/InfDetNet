...
def load_rgb_frames(image_dir, vid):
    ...
    for i in range(1, nf+1):
        try:
            ...
            print(os.path.join(image_dir, vid, 'img_'+str(i).zfill(5)+'.jpg'))
            raise e
        w,h,c = img.shape
        if w < 226 or h < 226:
            d = 226.-min(w,h)
...
def load_flow_frames(image_dir, vid):
    ...
    for i in range(1, nf+1):
        imgx = cv2.imread(os.path.join(image_dir, vid, 'flow_x_' + str(i).zfill(5) + '.jpg'), cv2.IMREAD_GRAYSCALE)
        imgy = cv2.imread(os.path.join(image_dir, vid, 'flow_y_' + str(i).zfill(5) + '.jpg'), cv2.IMREAD_GRAYSCALE)
...
class InfVis(data_utl.Dataset):
    def __init__(self, split_file, split, rgb_root, flow_root, classes, transf=None, down_ratio=4):
        with open(split_file) as f:
            split_dict = json.load(f)
            self.vid_list = [x for x in split_dict[split] if x not in ['000052_jump','000088_wave1',
                                                                       '000100_punch','000084_jump']]
...
def mt_collate_fn(batch):
    ...
    for b in batch:
        ...
        m = np.zeros(max_fea_len, np.float32)
        f_x[:b[0].shape[0]] = b[0]
        f_y[:b[1].shape[0]] = b[1]
        m[:b[0].shape[0]//8] = 1
